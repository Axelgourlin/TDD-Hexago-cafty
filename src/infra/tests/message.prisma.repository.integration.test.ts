...
describe("PrismaMessageRepository", () => {
  ...
  test("save() should save an existing message", async () => {
    ...
    await messageRepository.save(aliceMessageBuilder.withText(messageEditedText).build());
  ...
  test("getAllOfUser() should get all user messages", async () => {
    ...
    await Promise.all([aliceMessage, aliceMessage2, bobMessage].map((message) => messageRepository.save(message)));

    const expectedMessages = await messageRepository.getAllByAuthor("Alice");
    expect(expectedMessages).toHaveLength(2);
    expect(expectedMessages).toEqual(expect.arrayContaining([aliceMessage, aliceMessage2]));
  });
});
